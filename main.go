...
func main() {
	...
	destDlg := dialog.NewFolderOpen(func(uri fyne.ListableURI, err error) {
		if err == nil && uri != nil {
			...
			outputPathLabel.SetText(truncatePath(outputFolderPath, labelMaxChars))
		}
	}, myWindow)
	destDlg.Resize(fyne.NewSize(650, 450))

	openDlg := dialog.NewFileOpen(func(reader fyne.URIReadCloser, err error) {
		if err == nil && reader != nil {
			inputFilePath = reader.URI().Path()
			inputPathLabel.SetText(truncatePath(inputFilePath, labelMaxChars))
	...
	openDlg.Resize(fyne.NewSize(650, 450))
